...
// Open the element clicked 	
	...
	{
	 if($tablediv[index].id==($(this).text().trim()))
		{
 ...
 $( ".tc" ).on('click',function() 
{
		 ...
		 $("#"+$senarios[index].id).slideDown(0, function()
			{
				...
				$(this).addClass("white_content").css("display","inline-block");
				$(".senario").css("display","inline-block");
 ...
 $( "#load_log" ).on('click',function() 
	{
		 window.open('SeleniumLogs.html', '_blank');
	});
 
 
 $(".screens").on('click',function()
 {
	$image=$(this).attr("path");
	$('#imagefade').css("display","block");
	$('#imageviewer').css("display",'block');
	$('#imageviewer').html("<img id='imgsrc' src='file:\\"+$image+"'/><a id='closeImage' href='#' style='display:block'>close</a>");
	//$('#imageviewer').html("<div id='closeimg'><img id='closeImage' src='.//close.png'/></div>");
	//$('#closeimg').css('display','block');
	$('#closeImage').css('display','block');
 });
 
 $('#imagefade').on('click',function()
 {
	$('#closeImage').css('display','none');
	$('#imageviewer').css("display",'none');
	$('#imagefade').css("display","none");
